...
\begin{frame}[shrink=6]{Encontrar el Pico No es Buscar el Máximo}
  \vspace{-8pt}
  \begin{center}
    \includegraphics[height=0.50\textheight]{fig_encontrar_pico.png}
  \end{center}
  \vspace{-4pt}
  {\footnotesize\color{textMuted}
   El mes de mayor producción es casi siempre un \textbf{mes con suerte}: un pozo
   nuevo que entró, un mes sin paradas. Por eso buscamos el máximo sobre la
   producción \textbf{suavizada a seis meses}, que es la que refleja al campo
   y no al calendario.}
\end{frame}

\begin{frame}[shrink=8]{Cuánto Importa Esta Decisión}
  \vspace{-4pt}
  {\small\color{arcaDark}\bfseries
  ...
  \begin{columns}[T]
    \begin{column}{0.31\textwidth}
      \begin{tcolorbox}[colback=arcaGray, colframe=arcaRed, boxrule=0pt, leftrule=3pt,
        ...
        \centering
        {\scriptsize\color{textMuted}EL MÁXIMO CRUDO Y EL\\SUAVIZADO COINCIDEN EN}\\[4pt]
        {\LARGE\color{arcaRed}\bfseries 2 de 66}\\[4pt]
        {\scriptsize\color{textMuted}campos. En Statfjord\\se separan 58 meses}
      \end{tcolorbox}
    \end{column}
    \begin{column}{0.31\textwidth}
      \begin{tcolorbox}[colback=arcaGray, colframe=codeOrange, boxrule=0pt, leftrule=3pt,
        arc=3pt, left=6pt, right=6pt, top=6pt, bottom=6pt]
        \centering
        {\scriptsize\color{textMuted}NO TIENEN PICO,\\TIENEN MESETA}\\[4pt]
        {\LARGE\color{codeOrange}\bfseries 11 de 66}\\[4pt]
        {\scriptsize\color{textMuted}más de un año dentro\\del 95 \% del máximo}
      \end{tcolorbox}
    \end{column}
    \begin{column}{0.31\textwidth}
      \begin{tcolorbox}[colback=arcaGray, colframe=arcaDark, boxrule=0pt, leftrule=3pt,
        arc=3pt, left=6pt, right=6pt, top=6pt, bottom=6pt]
        \centering
        {\scriptsize\color{textMuted}VUELVEN A SUBIR\\3+ AÑOS DESPUÉS}\\[4pt]
        {\LARGE\color{arcaDark}\bfseries 7 de 66}\\[4pt]
        {\scriptsize\color{textMuted}pozos nuevos: el campo\\tiene dos picos}
      \end{tcolorbox}
    \end{column}
  \end{columns}
  \vspace{9pt}
  {\footnotesize\color{arcaDark}\itshape
   En esos casos «el pico» \textbf{no es un dato: es una decisión}. Y de esa
   decisión salen los cinco años que vamos a ver y los análogos con los que
   nos vamos a comparar. Conviene tomarla despierto.}
\end{frame}

\begin{frame}[shrink=6]{Por Eso se Alinea Desde el Pico}
  \vspace{-8pt}
  \begin{center}
    \includegraphics[height=0.56\textheight]{fig_alinear.png}
  \end{center}
  \vspace{-4pt}
  {\footnotesize\color{textMuted}
   Ekofisk arrancó en 1971 y Grane en 2003: por calendario, uno se está
   muriendo cuando el otro nace. Poniendo el cero en \textbf{el pico de cada
   uno}, los cuatro cuentan la misma historia.}
\end{frame}
...
\begin{frame}[shrink=4]{Dos Caminos, y Hay que Elegir Uno}
  \vspace{-10pt}
  \begin{center}
    \includegraphics[height=0.54\textheight]{fig_dos_caminos.png}
  \end{center}
  \vspace{-6pt}
  {\footnotesize\color{textMuted}
   A la izquierda se ajusta con \texttt{polyfit} y sale la
   {\color{arcaRed}\bfseries exponencial}; a la derecha con \texttt{curve\_fit}
   y sale la {\color{codeGreen}\bfseries hiperbólica}.
   {\color{arcaDark}La recta es más simple, pero \textbf{obliga} al campo a perder
   el mismo porcentaje todos los años; la curva le permite ir frenando.}}
\end{frame}

\begin{frame}[shrink=6]{Dos Caminos, y Hay que Elegir Uno}
  \vspace{-8pt}
  \begin{center}
    \includegraphics[height=0.54\textheight]{fig_dos_caminos.png}
  \end{center}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      {\footnotesize\color{textMuted}
      {\color{arcaRed}\bfseries Camino 1.} Doblo el eje con el logaritmo, la curva
      se endereza, y ajusto una \textbf{recta}. Eso es la \textbf{exponencial},
      y se hace con \texttt{polyfit}.}
    \end{column}
    \begin{column}{0.48\textwidth}
      {\footnotesize\color{textMuted}
      {\color{codeGreen}\bfseries Camino 2.} Dejo el eje en barriles y ajusto
      directamente una \textbf{curva}. Eso es la \textbf{hiperbólica}, y se hace
      con \texttt{curve\_fit}.}
    \end{column}
  \end{columns}
  \vspace{6pt}
  {\footnotesize\color{arcaDark}
   La recta es más simple, pero \textbf{obliga} al campo a perder el mismo
   porcentaje todos los años. La curva le permite ir frenando --- y ya vimos
   que \textbf{un tercio de los campos no se endereza} ni doblando el eje.}
\end{frame}
...
\begin{frame}[shrink=40]{Por Eso \texttt{curve\_fit} Pide Cosas que \texttt{polyfit} No}
  \vspace{-4pt}
  {\footnotesize
  \renewcommand{\arraystretch}{1.35}
  \begin{tabular}{@{}p{3.0cm}p{5.4cm}p{5.9cm}@{}}
    \toprule
    &
    {\color{codeBlue}\bfseries \texttt{polyfit} (la recta)} &
    {\color{codeOrange}\bfseries \texttt{curve\_fit} (la hiperbólica)} \\
    \midrule
    {\color{arcaDark}\bfseries Qué minimiza} &
      La suma de errores al cuadrado &
      Exactamente lo mismo \\
    {\color{arcaDark}\bfseries Cómo lo hace} &
      Con una \textbf{fórmula}: lo despeja &
      \textbf{Caminando} cuesta abajo, paso a paso \\
    {\color{arcaDark}\bfseries ¿Punto de partida?} &
      No hace falta &
      Sí: \texttt{p0}. Si arranca lejos, puede terminar en otro lado \\
    {\color{arcaDark}\bfseries ¿Límites?} &
      No hace falta &
      Sí: \texttt{bounds}. Sin ellos se va a valores sin sentido físico \\
    \rowcolor{arcaGray}
    {\color{arcaDark}\bfseries ¿Siempre da lo mismo?} &
      {\color{codeGreen}\bfseries Sí, siempre} &
      {\color{arcaRed}\bfseries No necesariamente} \\
    \bottomrule
  \end{tabular}}
  \vspace{8pt}
  ...
  \begin{tcolorbox}[colback=arcaGray, colframe=arcaRed, boxrule=0pt, leftrule=3pt,
    ...
    {\scriptsize\color{arcaRed}\bfseries\faExclamationTriangle\hspace{4pt}Y UNA CONSECUENCIA INCÓMODA}\\[4pt]
    {\scriptsize\color{textMuted}
     Como $b$ y $D_i$ aparecen \textbf{multiplicados} dentro del paréntesis, se
     compensan: un $D_i$ más alto con un $b$ más alto da casi la misma curva.
     Con solo cinco años de datos el ajuste no los distingue bien.\\[3pt]
     Por eso dos ingenieros pueden ajustar el mismo campo, reportar \emph{b}
     distintos, y ninguno estar equivocado.}
  \end{tcolorbox}
\end{frame}
