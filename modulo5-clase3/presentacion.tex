...
\begin{frame}[shrink=6]{Antes de Ajustar Nada: ¿Qué Quiere Decir «Ajustar»?}
  \vspace{-8pt}
  \begin{center}
    \includegraphics[height=0.52\textheight]{fig_que_es_ajustar.png}
  \end{center}
  \vspace{-4pt}
  {\footnotesize\color{arcaDark}
   No hay magia: se prueban rectas y se elige la que deja las barras de abajo
   lo más chicas posible. Los errores se \textbf{elevan al cuadrado} para que
   un error grande pese mucho más que dos chicos --- porque en ingeniería una
   equivocación grande no se compensa con dos aciertos.}
\end{frame}

\begin{frame}[shrink=6]{Dos Formas de Buscar el Fondo}
  \vspace{-8pt}
  \begin{center}
    \includegraphics[height=0.52\textheight]{fig_dos_ajustes.png}
  \end{center}
  \vspace{-4pt}
  {\footnotesize\color{textMuted}
   Cada punto del mapa es una combinación de parámetros; el color es el error
   que da. \textbf{Ajustar es encontrar el fondo.} A la izquierda el fondo se
   despeja con álgebra; a la derecha hay que caminar cuesta abajo desde algún
   lado.}
\end{frame}
...
\begin{frame}[shrink=40]{Por Eso \texttt{curve\_fit} Pide Cosas que \texttt{polyfit} No}
  ...
  \begin{tcolorbox}[colback=arcaGray, colframe=arcaRed, boxrule=0pt, leftrule=3pt,
    ...
    {\scriptsize\color{arcaRed}\bfseries\faExclamationTriangle\hspace{4pt}Y MIREN EL VALLE DE LA LÁMINA ANTERIOR}\\[4pt]
    {\scriptsize\color{textMuted}
     Es \textbf{largo y torcido}: hay combinaciones de $D_i$ y $b$ muy distintas
     con casi el mismo error. Un $D_i$ más alto se compensa con un $b$ más alto.
     Por eso dos ingenieros pueden ajustar el mismo campo y reportar
     \emph{b} distintos sin que ninguno se haya equivocado.}
  \end{tcolorbox}
\end{frame}
