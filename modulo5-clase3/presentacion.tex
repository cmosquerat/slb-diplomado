...
\begin{frame}[shrink=6]{Qué es una Curva de Declinación}
  \vspace{-6pt}
  \begin{center}
    \includegraphics[height=0.62\textheight]{fig_que_es_declinacion.png}
  \end{center}
  \vspace{-2pt}
  {\footnotesize\color{textMuted}
...
\begin{frame}[shrink=6]{El Encargo, en una Figura}
  \vspace{-6pt}
  \begin{center}
    \includegraphics[height=0.68\textheight]{fig_el_encargo.png}
  \end{center}
  \vspace{-4pt}
...
URL = ("https://raw.githubusercontent.com/"
       "cmosquerat/slb-diplomado/main/datos/"
       "campos_noruega_declinacion.csv")

d = pd.read_csv(URL)

# cuantos campos y cuanta historia
print(d.campo.nunique(), "campos")
print(len(d), "filas (una por campo y mes)")

# los anios de historia de cada campo
...
\begin{frame}[shrink=8]{Quién Fue Arps, y Qué Escribió}
  \vspace{-4pt}
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      {\footnotesize\color{textMuted}
...
\begin{frame}[shrink=6]{La Exponencial: la Recta de la Clase 1, Con Nombre}
  \vspace{-6pt}
  \begin{center}
    \includegraphics[height=0.62\textheight]{fig_que_es_arps.png}
  \end{center}
  \vspace{-2pt}
  {\footnotesize\color{textMuted}
...
q = campos["DRAUGEN"]        # bbl/dia, desde el pico
y = q.iloc[:60].values       # los primeros 5 anios
t = np.arange(60)            # 0, 1, 2, ... 59

# ajustamos una RECTA sobre el logaritmo.
# polyfit con grado 1 devuelve [pendiente, altura]
b = np.polyfit(t, np.log(y), 1)

# la pendiente es -D mensual: la pasamos a anual
D_anual = 100 * (1 - np.exp(b[0] * 12))
print("declina", round(D_anual), "% al anio")

# proyectamos 12 anios = 144 meses
pron = np.exp(np.polyval(b, np.arange(144)))

# ojo: pron es un CAUDAL (bbl/dia). Para acumular
# barriles hay que multiplicar por los dias del mes
acum = pron.sum() * 30.4
print("acumulado:", round(acum / 1e6, 1), "MMbbl")
      \end{pyblock}
    \end{column}
...
\begin{frame}[shrink=6]{Ahora Destapamos: ¿Cuánto se Equivocó?}
  \vspace{-6pt}
  \begin{center}
    \includegraphics[height=0.62\textheight]{fig_sesgo.png}
  \end{center}
  \vspace{-2pt}
  {\footnotesize\color{arcaDark}
...
\begin{frame}[shrink=8]{Error y Sesgo No Son lo Mismo}
  \vspace{-4pt}
  {\small\color{arcaDark}\bfseries
...
\begin{frame}[shrink=6]{La Hiperbólica: Qué es b}
  \vspace{-6pt}
  \begin{center}
    \includegraphics[height=0.62\textheight]{fig_que_es_b.png}
  \end{center}
  \vspace{-2pt}
  {\footnotesize\color{textMuted}
...
# la formula de Arps, tal cual la escribio en 1945
def hiperbolica(t, qi, Di, b):
    return qi / (1 + b * Di * t) ** (1 / b)

# curve_fit busca los tres numeros que mejor
# ajustan. Le damos un punto de partida (p0) y
# limites para que no se vaya a valores absurdos
p, _ = curve_fit(
    hiperbolica, t, y,
    p0=[y[0], 0.02, 0.5],
...
pron = hiperbolica(np.arange(144), *p)
print("acumulado:", round(pron.sum() * 30.4 / 1e6, 1))
      \end{pyblock}
    \end{column}
    \begin{column}{0.40\textwidth}
      \vspace{2pt}
...
\begin{frame}[shrink=6]{Las Dos Curvas, y lo que Pasó}
  \vspace{-6pt}
  \begin{center}
    \includegraphics[height=0.66\textheight]{fig_ajuste_comparado.png}
  \end{center}
  \vspace{-4pt}
  {\footnotesize\color{arcaDark}
...
\begin{frame}[shrink=8]{El Campo No Produce Hasta Cero}
  \vspace{-4pt}
  {\small\color{arcaDark}\bfseries
...
\begin{frame}[shrink=6]{La Curva Elegida Decide el Año de Cierre}
  \vspace{-6pt}
  \begin{center}
    \includegraphics[height=0.66\textheight]{fig_limite_economico.png}
  \end{center}
  \vspace{-4pt}
  {\footnotesize\color{arcaDark}
...
\begin{frame}[shrink=8]{La Trampa de la Cola: Campos Inmortales}
  \vspace{-4pt}
  {\small\color{arcaDark}\bfseries
...
\begin{frame}[shrink=6]{El Sesgo del 7 \%, en el Idioma de Gerencia}
  \vspace{-6pt}
  \begin{center}
    \includegraphics[height=0.60\textheight]{fig_dolares.png}
  \end{center}
  \vspace{-2pt}
  {\footnotesize\color{textMuted}
...
\begin{frame}[shrink=8]{Qué Cambia una Decisión, y Qué No}
  \vspace{-4pt}
  {\small\color{arcaDark}\bfseries
...
\begin{frame}[shrink=6]{El Marcador de los 54 Campos}
  \vspace{-6pt}
  \begin{center}
    \includegraphics[height=0.64\textheight]{fig_marcador.png}
  \end{center}
  \vspace{-2pt}
  {\footnotesize\color{arcaDark}
...
\begin{frame}[shrink=8]{La Pregunta Incómoda}
  \vspace{6pt}
...
\begin{frame}[shrink=8]{Qué Significan P10, P50 y P90}
  \vspace{-4pt}
  {\small\color{arcaDark}\bfseries
...
\begin{frame}[shrink=8]{De Dónde Sale la Banda: la Experiencia Ajena}
  \vspace{-4pt}
  {\small\color{arcaDark}\bfseries
...
\begin{frame}[shrink=6]{Los Análogos, en Dos Figuras}
  \vspace{-6pt}
  \begin{center}
    \includegraphics[height=0.62\textheight]{fig_analogos.png}
  \end{center}
  \vspace{-2pt}
  {\footnotesize\color{textMuted}
...
\begin{frame}[shrink=6]{Las Tres Respuestas, y el Resultado}
  \vspace{-6pt}
  \begin{center}
    \includegraphics[height=0.66\textheight]{fig_banda.png}
  \end{center}
  \vspace{-4pt}
  {\footnotesize\color{arcaDark}
...
\begin{frame}[shrink=6]{¿Cumple lo que Promete?}
  \vspace{-6pt}
  \begin{center}
    \includegraphics[height=0.68\textheight]{fig_calibracion.png}
  \end{center}
  \vspace{-4pt}
  {\footnotesize\color{arcaDark}
...
\begin{frame}[shrink=8]{Su Turno: Firmen un Número \hfill {\normalsize\color{arcaRed}\faClock\hspace{4pt}25 min}}
  \vspace{-4pt}
  \begin{tcolorbox}[colback=arcaGray, colframe=arcaDark, boxrule=0pt, leftrule=3pt,
    arc=3pt, left=8pt, right=8pt, top=5pt, bottom=5pt]
    {\footnotesize\color{arcaDark}\itshape
