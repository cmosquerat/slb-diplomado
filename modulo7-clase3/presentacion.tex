...
\begin{frame}[shrink=5]{\faTools\hspace{4pt}Su Reto · Fashion-MNIST}
  \vspace{4pt}
  \begin{tcolorbox}[colback=white, colframe=arcaRed, boxrule=1.4pt,
    arc=3pt, left=9pt, right=9pt, top=6pt, bottom=6pt,
    title={\hspace{-2pt}\faTools\hspace{5pt}SU ENCARGO},
    ...
    boxed title style={arc=2pt, boxrule=0pt, top=1.5pt, bottom=1.5pt,
      left=6pt, right=6pt}]
    {\footnotesize\color{textMuted}
     El mismo problema, otro dato: \textbf{Fashion-MNIST} --- 70.000
     prendas de ropa en imágenes de 28×28.\\[5pt]
     \textbf{El objetivo:} repitan lo que acaban de hacer con los dígitos
     --- la red, su examen y sus errores --- sobre este dato. La carga
     viene dada en el cuaderno; el resto es de ustedes.}
  \end{tcolorbox}
  \vspace{7pt}
  {\footnotesize\color{arcaDark}\bfseries\itshape
   En clase si alcanza; si no, para la casa. Sin solución escrita, como
   siempre.}
\end{frame}
